...
class Transformer(nn.Module):
    def __init__(self, input_l, output_l, n_token, encoder_layer=6, decoder_layer=6, d=512, n_head=8, sos_id=1,
                 pad_id=0):
        super().__init__()
        self.encoder = Encoder(input_l, n_token, n_layer=encoder_layer, d=d, n_head=n_head, pad_id=pad_id)
        self.decoder = Decoder(output_l, input_l, n_token, n_layer=decoder_layer, d=d, n_head=n_head, sos_id=sos_id,
                               pad_id=pad_id)
...
class Encoder(nn.Module):
    ...
    def forward(self, inputs):
        ...
        output = self.transformer_encoder(src=source_embed, mask=attn_mask,
                                          src_key_padding_mask=padding_mask)  # [S, B, 512]
        output = torch.transpose(output, -2, -3)  # [B, S, 512]
        return output


class Decoder(nn.Module):
    def __init__(self, max_l, input_l, n_token, sos_id=1, pad_id=0, n_layer=6, n_head=8, d=512):
        super().__init__()
        self.pad_id = pad_id
        self.sos_id = sos_id
        self.n_head = n_head
    ...
    def forward(self, source, caption, top_k=1, eos_id=2, mode='greedy'):
        ...
        posit_index = torch.arange(caption.shape[1]).unsqueeze(0).repeat(caption.shape[0], 1).to(
            caption.device)  # (B,L)
        ...
        attn_mask = torch.triu(torch.full((caption.shape[1], caption.shape[1]), float('-inf')), diagonal=1).to(
            caption.device)  # [L,L]
        ...
        out = self.transformer_decoder(tgt=target_embed, memory=source_embed, tgt_mask=attn_mask,
